...
export default function Home() {
  ...
  const sendMessage = async (message) => {
    ...
    const newMessage = { id: Date.now(), text: trimmedMessage, sender: "You" };
    ...
    setMessages((messages) => [
      ...messages,
      { id: Date.now() + 1, text: formattedReply, sender: "Gemini Bot" },
    ]);
  };
      ...
      <div className={styles.chatArea}>
        ...
        {showQuickResponses && (
          <div className={styles.quickResponses}>
            <p className="style: text-align: center;">Choose a quick response: </p>            
            <button onClick={() => quickResponse("Hello!")}>Hello</button>
            <button
              onClick={() =>
                quickResponse("Give a quick tip for developer please")
              }
            >
              Quick Tip
            </button>
